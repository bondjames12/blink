...
//                Syncro.getInstance().refreshDevices();
    ...
    private Attribute getAttributeByDescription(String description) {
        for (Attribute attribute : getAttributes()) {
            if(attribute.getAttributeType().getDescription().compareToIgnoreCase(description) == 0)
            {
                return attribute;
            }
        }
        //if we got to this point we didn't find the attribute we needed. This is a BIG problem!
        throw new NullPointerException();
    }

    private Attribute getOnOffAttribute() {
        if(getInterconnect().compareToIgnoreCase("lutron") == 0)
        {
            return getAttributes().get(0);
        }
        if(getInterconnect().compareToIgnoreCase("zigbee") == 0)
        {
            return getAttributes().get(0);
        }
    ...
    private Attribute getLevelAttribute() {
        if(this.getInterconnect().compareToIgnoreCase("lutron") == 0)
        {
            return getAttributes().get(0);
        }
        if(this.getInterconnect().compareToIgnoreCase("zigbee") == 0)
        {
    ...
    public void setLevel(int level) {
        if (getLevelAttribute().getInt() != level) {
            getLevelAttribute().setValueLocal(level);
    ...
    public void setOn(boolean on) {
        if(this.getInterconnect().compareToIgnoreCase("lutron") == 0)
        {
            getLevelAttribute().setValueLocal(on ? "255" : "0");
            state = BlinkApp.STATE_UPDATED;
            update();
            return;
        }
        if (on != getOnOffAttribute().getBool()) {
            getOnOffAttribute().setValueLocal(on);
            state = BlinkApp.STATE_UPDATED;
            update();
        }
    }
    ...
    public boolean isOn() {
        ...
        if(getOnOffAttribute() == null) {
            return false;
        }
        return getOnOffAttribute().getBool();
    }

    public int getLevel() {
        if(getLevelAttribute() == null) {
            return 0;
        }
        return getLevelAttribute().getInt();
    }
